    ...
    public class BoardLoaderService(MainViewModel viewModel)
    {
        public async Task LoadObfOrObzFileAsync(string filePath)
        {
            ...
            else if (filePath.EndsWith(".obf", StringComparison.OrdinalIgnoreCase))
            {
                ...
                AppLogger.LogInfo(
                    Resources.BoardLoaderService_LoadObfOrObzFileAsync_Nieobsługiwany_typ_pliku__Podaj_plik_z_rozszerzeniem__obf_lub__obz_);
        }
        ...
        public async Task LoadObfFileAsync(string? filePath)
        {
            ...
            catch (Exception ex)
            {
                AppLogger.LogError(string.Format(
                    Resources.BoardLoaderService_LoadObfFileAsync_Błąd_podczas_ładowania_pliku_OBF___0___Szczegóły___1_, filePath, ex.Message));
            }
            finally
            {
        ...
        private async Task LoadObzFileAsync(string filePath)
        {
            ...
            {
                AppLogger.LogError(string.Format(Resources.BoardLoaderService_LoadObzFileAsync_Error_loading_OBZ_file___0_, ex.Message));
            }
            finally
            {
                viewModel.IsLoading = false;
            ...
            await Task.Run(() =>
            {
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    else
                    {
                        ...
                        {
                            AppLogger.LogError(string.Format(
                                Resources.BoardLoaderService_ExtractObzArchiveAsync_Access_denied_to_file___0____1_, destinationPath, ex.Message));
                        }
                        catch (IOException ex)
                        {
                            AppLogger.LogError(string.Format(
                                Resources.BoardLoaderService_ExtractObzArchiveAsync_Unable_to_overwrite_file___0____1_, destinationPath, ex.Message));
                        }
                        catch (Exception ex)
                        {
                            AppLogger.LogError(string.Format(
                                Resources.BoardLoaderService_ExtractObzArchiveAsync_An_error_occurred_while_extracting_file___0____1_, entry.FullName, ex.Message));
                        }
                    }
                }
            });
