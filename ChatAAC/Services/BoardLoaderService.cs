...
namespace ChatAAC.Services
{
    public class BoardLoaderService(MainViewModel viewModel)
    {
        ...
        public async Task LoadObfOrObzFileAsync(string filePath)
        {
            ...
            {
                AppLogger.LogInfo(Resources.BoardLoaderService_LoadObfOrObzFileAsync_Nieobsługiwany_typ_pliku__Podaj_plik_z_rozszerzeniem__obf_lub__obz_);
            }
        }
            ...
            try
            {
                    ...
                    || viewModel.ObfFileHistory[viewModel.CurrentHistoryIndex] != filePath)
                {
                    if (viewModel.CurrentHistoryIndex < viewModel.ObfFileHistory.Count - 1)
                        viewModel.ObfFileHistory = viewModel.ObfFileHistory.Take(viewModel.CurrentHistoryIndex + 1).ToList();
            ...
            try
            {
                ...
                if (File.Exists(manifestPath))
                {
                    ...
                    if (File.Exists(rootObfPath))
                        ...
                        AppLogger.LogInfo(Resources.BoardLoaderService_LoadObzFileAsync_root_obf_file_not_found_in_the_OBZ_package_);
                }
                else
                {
                    AppLogger.LogInfo(Resources.BoardLoaderService_LoadObzFileAsync_manifest_json_file_not_found_in_the_OBZ_package_);
                }
            }
            catch (Exception ex)
            {
            ...
            await Task.Run(() =>
            {
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    if (destinationDirectory == null)
                        continue;
                    var destinationPath = Path.GetFullPath(Path.Combine(destinationDirectory, Path.GetFileName(sanitizedEntryName)));
                    if (!destinationPath.StartsWith(destinationDirectory, StringComparison.Ordinal))
                    {
                        AppLogger.LogInfo(string.Format(Resources.BoardLoaderService_ExtractObzArchiveAsync_Skipped_unsafe_entry___0_, entry.FullName));
                        continue;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            AppLogger.LogError(string.Format(Resources.BoardLoaderService_ExtractObzArchiveAsync_Access_denied_to_file___0____1_,
                                destinationPath, ex.Message));
                        }
                        catch (IOException ex)
                        {
                            AppLogger.LogError(string.Format(Resources.BoardLoaderService_ExtractObzArchiveAsync_Unable_to_overwrite_file___0____1_,
                                destinationPath, ex.Message));
                        }
                        catch (Exception ex)
                        {
                            AppLogger.LogError(string.Format(Resources.BoardLoaderService_ExtractObzArchiveAsync_An_error_occurred_while_extracting_file___0____1_,
        ...
        private void LoadButtonsFromObfData(ObfFile obfFile)
        {
            ...
            if (obfFile.Grid != null)
            {
                ...
                foreach (var row in obfFile.Grid.Order)
                {
                    ...
                    foreach (var buttonId in row)
                    {
                        ...
                        viewModel.GridCells.Add(new GridCellViewModel(rowIndex, columnIndex, btn, viewModel));
                        columnIndex++;
                    }
                    rowIndex++;
                }
            ...
            else
            {
                ...
                {
                    viewModel.GridCells.Add(new GridCellViewModel(0, col, btn, viewModel));
                    col++;
                }
                viewModel.GridRows = 1;
                viewModel.GridColumns = obfFile.Buttons.Count;
            }
        }
